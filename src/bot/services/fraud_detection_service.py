...
class FraudDetectionService:
    ...
    def __init__(self):
        ...
        self.config = {
            ...
            'blacklist_threshold': 5,  # 5+ tentativas = "blacklist_global"
    ...
    async def validate_invite(self, invited_user_id: int, inviter_user_id: int, 
        ...
        try:
            ...
            return FraudDetectionResult(
                ...
                confidence=1.0,
                metadata={'validation_timestamp': TIMESTAMP WITH TIME ZONE.now().isoformat()}
            )
    ...
    async def _detect_coordinated_attack(self, invited_user_id: int, inviter_user_id: int, 
        ...
        try:
            ...
            similar_users_global_global = await self._find_users_global_global_with_similar_pattern(invited_user_id, competition_id)
            
            if len(similar_users_global_global) >= self.config['coordinated_attack_threshold']:
                await self._create_fraud_alert(
                    invited_user_id,
                    FraudType.COORDINATED_ATTACK,
                    0.9,
                    {'similar_users_global_global_count': len(similar_users_global_global), 'user_ids': similar_users_global_global[:10]},
                    "Possível ataque coordenado detectado"
                )
                
                return FraudDetectionResult(
                    is_valid=False,
                    reason=f"Ataque coordenado detectado: {len(similar_users_global_global)} usuários com padrão similar",
                    confidence=0.9,
                    metadata={'coordinated_users_global_global': len(similar_users_global_global)}
                )
    ...
    async def _find_users_global_global_with_similar_pattern(self, user_id: int, competition_id: int) -> List[int]:
        ...
        try:
            async with self.db.async_session_factory() as session:
                ...
                result = await session.execute(text(similar_query, {
    ...
    async def _update_user_profile(self, user_id: int, action: str, metadata: Dict[str, Any]):
        ...
        try:
            ...
            if action == 'valid_invite':
                ...
                profile.suspicious_indicators.append(f"suspicious_{TIMESTAMP WITH TIME ZONE.now().isoformat()}")
    ...
    async def _create_fraud_alert(self, user_id: int, fraud_type: FraudType, 
        ...
        try:
            ...
            # Se confiança alta, considerar "blacklist_global" automático
            if confidence >= 0.9:
    ...
    async def _auto_blacklist_user(self, user_id: int, fraud_type: FraudType, details: Dict[str, Any]):
        """blacklist_global" automático para usuários com alta confiança de fraude"""
        try:
            async with self.db.async_session_factory() as session:
                from sqlalchemy import VARCHAR
            ...
            logger.error(f"Erro no "blacklist_global" automático: {e}")
        ...
        try:
            async with self.db.async_session_factory() as session:
                ...
                return {
                    ...
                    'blacklisted_users_global_global': await self._count_blacklisted_users_global_global()
                }
    ...
    async def _count_blacklisted_users_global_global(self) -> int:
        """Conta usuários na blacklist"""
        try:
            async with self.db.async_session_factory() as session:
                from sqlalchemy import VARCHAR
                
                count_query = text("SELECT COUNT(*) FROM users_global_global_optimized WHERE is_blacklisted = TRUE")
