            ...
            async with self.db.async_session_factory() as session:
                ...
                ranking_query = text("""
                    WITH ranked_participants AS (
                        ...
                        INNER JOIN users_global_global_optimized u ON cp.user_id = u.user_id
            ...
            async with self.db.async_session_factory() as session:
                position_query = text("""
                    WITH user_rank AS (
                        ...
                        FROM competition_participants_global_global_optimized
                ...
                result = await session.execute(text(position_query, {
                    'competition_id': competition_id,
                    'user_id': user_id
                })
    ...
    async def get_competition_stats_fast(self, competition_id: int) -> Dict[str, Any]:
        ...
        try:
            async with self.db.async_session_factory() as session:
                ...
                result = await session.execute(text(stats_query, {'competition_id': competition_id})
                ...
                if competition_id:
                    ...
                    user_stats_query = text("""
                        SELECT 
                            ...
                            (
                                SELECT COUNT(*) 
                                FROM invite_links_global_global_optimized il 
                            ...
                            (
                                SELECT COUNT(*)
                                FROM global_global_unique_invited_users_global_global uiu
                        ...
                        FROM users_global_global_optimized u
                        LEFT JOIN competition_participants_global_global_optimized cp ON (
                            u.user_id = cp.user_id AND cp.competition_id = :competition_id
                        )
                        WHERE u.user_id = :user_id
                    """)
                    
                    result = await session.execute(text(user_stats_query, {
                        'user_id': user_id,
                        'competition_id': competition_id
                    })
                else:
                    ...
                    user_stats_query = text("""
                        SELECT 
                            ...
                            ) as competitions_global_global_participated
                        FROM users_global_global_optimized u
                        WHERE u.user_id = :user_id
                    """)
                    
                    result = await session.execute(text(user_stats_query, {'user_id': user_id})
                ...
                if competition_id:
                    history_query = text("""
                        SELECT 
                            ...
                            uiu.fraud_flags
                        FROM global_global_unique_invited_users_global_global uiu
                        JOIN users_global_global_optimized u ON uiu.invited_user_id = u.user_id
                    ...
                    result = await session.execute(text(history_query, {
                        'user_id': user_id,
                        'competition_id': competition_id,
                        'limit': limit
                    })
                    ...
                    history_query = text("""
                        ...
                        FROM global_global_unique_invited_users_global_global uiu
                        JOIN users_global_global_optimized u ON uiu.invited_user_id = u.user_id
                        JOIN competitions_global_global_optimized c ON uiu.competition_id = c.id
                        WHERE uiu.inviter_user_id = :user_id
                        ORDER BY uiu.first_join_timestamp DESC
                        LIMIT :limit
                    """)
    ...
    async def detect_suspicious_patterns_batch(self, user_ids: List[int], 
        ...
        try:
            async with self.db.async_session_factory() as session:
                ...
                result = await session.execute(text(patterns_query, {
                    'user_ids': user_ids,
                    'hours': hours_window
                })
    ...
    async def get_coordinated_attack_indicators(self, competition_id: int, 
        ...
        try:
            async with self.db.async_session_factory() as session:
                coordinated_query = text("""
                    WITH time_clusters AS (
                        SELECT 
                            ...
                            ARRAY_AGG(DISTINCT invited_user_id) as invited_users_global_global
                ...
                result = await session.execute(text(coordinated_query, {
                    'competition_id': competition_id,
                ...
                metrics_queries = {
                    'database_stats': text("""
                        SELECT 
                            ...
                            (SELECT COUNT(*) FROM competitions_global_global_optimized WHERE status = 'active') as active_competitions_global_global,
                            (SELECT SUM(valid_invites_count) FROM competition_participants_global_global_optimized) as total_valid_invites,
                            (SELECT COUNT(*) FROM global_global_unique_invited_users_global_global WHERE is_valid_invite = FALSE) as total_fraud_attempts
                    """),
                    ...
                    'table_stats': text("""
                        ...
                        WHERE tablename IN ('users_global_global_optimized', 'competition_participants_global_global_optimized', 'global_global_unique_invited_users_global_global')
                    """)
                }
    ...
    async def get_slow_queries_analysis(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            async with self.db.async_session_factory() as session:
                ...
                result = await session.execute(text(slow_queries_query, {'limit': limit})
                return [dict(row._mapping) for row in result]
    ...
    async def cleanup_old_data_optimized(self, days_to_keep: int = 90) -> Dict[str, int]:
        ...
        try:
            async with self.db.async_session_factory() as session:
                ...
                result = await session.execute(text(audit_cleanup, {'days': days_to_keep})
                cleanup_results['audit_logs_deleted'] = result.rowcount
                
                # Limpar detecções de fraude antigas
                fraud_cleanup = text("""
                    DELETE FROM fraud_detection_log_global_global_global_global_global_global_global_global_global_global_global_global_global 
                    WHERE timestamp < NOW() - INTERVAL ':days days'
                """)
                
                result = await session.execute(text(fraud_cleanup, {'days': days_to_keep})
