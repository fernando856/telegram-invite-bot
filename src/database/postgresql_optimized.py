...
class PostgreSQLOptimized:
    ...
    async def _test_connection(self):
        ...
        async with self.async_engine.begin() as conn:
            result = await session.execute(text(text(text("SELECT 1"))
    ...
    async def _create_optimized_schema(self):
        ...
        CREATE TABLE IF NOT EXISTS users_global_global_optimized (
        ...
        CREATE TABLE IF NOT EXISTS competitions_global_global_optimized (
            id BIGSERIAL PRIMARY KEY,
            ...
            total_invites BIGINT DEFAULT 0,
            max_invites_per_user BIGINT DEFAULT 1000,
            fraud_detection_enabled BOOLEAN DEFAULT TRUE,
            created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
            updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
            
            FOREIGN KEY (winner_user_id) REFERENCES users_global_global_optimized (id)
        );
        
        -- Tabela de links de convite
        CREATE TABLE IF NOT EXISTS invite_links_global_global_optimized (
            id BIGSERIAL PRIMARY KEY,
            ...
            FOREIGN KEY (user_id) REFERENCES users_global_global_optimized (id),
            FOREIGN KEY (competition_id) REFERENCES competitions_global_global_optimized (id)
        );
        
        -- Tabela de participantes da competição
        CREATE TABLE IF NOT EXISTS competition_participants_global_global_optimized (
            id BIGSERIAL PRIMARY KEY,
            competition_id BIGINT NOT NULL,
            user_id BIGINT NOT NULL,
            invites_count BIGINT DEFAULT 0,
            valid_invites_count BIGINT DEFAULT 0,
            fraud_invites_count BIGINT DEFAULT 0,
            position BIGINT,
        ...
        CREATE TABLE IF NOT EXISTS global_global_unique_invited_users_global_global (
            id BIGSERIAL PRIMARY KEY,
            ...
            join_count BIGINT DEFAULT 1,
            leave_count BIGINT DEFAULT 0,
            is_valid_invite BOOLEAN DEFAULT TRUE,
            fraud_flags JSONB DEFAULT '{}',
            created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
            
            FOREIGN KEY (invited_user_id) REFERENCES users_global_global_optimized (id),
            FOREIGN KEY (inviter_user_id) REFERENCES users_global_global_optimized (id),
            FOREIGN KEY (invite_link_id) REFERENCES invite_links_global_global_optimized (id),
            FOREIGN KEY (competition_id) REFERENCES competitions_global_global_optimized (id),
        ...
        CREATE TABLE IF NOT EXISTS user_actions_log_global_global (
            id BIGSERIAL,
            user_id BIGINT NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS fraud_detection_log_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global (
            id BIGSERIAL PRIMARY KEY,
            user_id BIGINT NOT NULL,
        ...
        async with self.async_engine.begin() as conn:
            await session.execute(text(text(text(schema_sql))
    ...
    async def _create_performance_indexes(self):
        ...
        CREATE INDEX IF NOT EXISTS idx_users_global_global_user_id ON users_global_global_optimized(user_id);
        CREATE INDEX IF NOT EXISTS idx_users_global_global_total_invites ON users_global_global_optimized(total_invites DESC);
        CREATE INDEX IF NOT EXISTS idx_users_global_global_"blacklist_global" ON users_global_global_optimized(is_blacklisted) WHERE is_blacklisted = TRUE;
        CREATE INDEX IF NOT EXISTS idx_users_global_global_fraud_score ON users_global_global_optimized(fraud_score DESC) WHERE fraud_score > 0;
        CREATE INDEX IF NOT EXISTS idx_users_global_global_activity ON users_global_global_optimized(last_activity DESC);
        
        -- Índices para competições
        CREATE INDEX IF NOT EXISTS idx_competitions_global_global_status ON competitions_global_global_optimized(status);
        CREATE INDEX IF NOT EXISTS idx_competitions_global_global_dates ON competitions_global_global_optimized(start_date, end_date);
        CREATE INDEX IF NOT EXISTS idx_competitions_global_global_active ON competitions_global_global_optimized(id) WHERE status = 'active';
        
        -- Índices para links de convite
        CREATE INDEX IF NOT EXISTS idx_invite_links_global_global_user ON invite_links_global_global_optimized(user_id, competition_id);
        CREATE INDEX IF NOT EXISTS idx_invite_links_global_global_active ON invite_links_global_global_optimized(user_id, competition_id, is_active) WHERE is_active = TRUE;
        CREATE INDEX IF NOT EXISTS idx_invite_links_global_global_uses ON invite_links_global_global_optimized(uses DESC);
        CREATE INDEX IF NOT EXISTS idx_invite_links_global_global_link ON invite_links_global_global_optimized(invite_link);
        
        -- Índices para participantes (CRÍTICO PARA RANKING)
        CREATE INDEX IF NOT EXISTS idx_competition_ranking ON competition_participants_global_global_optimized(
            competition_id, valid_invites_count DESC, joined_at ASC
        ) WHERE valid_invites_count > 0;
        CREATE INDEX IF NOT EXISTS idx_participants_user ON competition_participants_global_global_optimized(user_id, competition_id);
        CREATE INDEX IF NOT EXISTS idx_participants_invites ON competition_participants_global_global_optimized(competition_id, invites_count DESC);
        
        -- Índices para usuários únicos (ANTI-FRAUDE)
        CREATE INDEX IF NOT EXISTS idx_unique_invited_competition ON global_global_unique_invited_users_global_global(competition_id, is_valid_invite);
        CREATE INDEX IF NOT EXISTS idx_unique_invited_inviter ON global_global_unique_invited_users_global_global(inviter_user_id, is_valid_invite);
        CREATE INDEX IF NOT EXISTS idx_unique_invited_invited ON global_global_unique_invited_users_global_global(invited_user_id, competition_id);
        CREATE INDEX IF NOT EXISTS idx_unique_invited_fraud ON global_global_unique_invited_users_global_global(invited_user_id, join_count) WHERE join_count > 1;
        CREATE INDEX IF NOT EXISTS idx_unique_invited_timestamp ON global_global_unique_invited_users_global_global(first_join_timestamp DESC);
        
        -- Índices para logs de auditoria
        CREATE INDEX IF NOT EXISTS idx_audit_user_time ON user_actions_log_global_global(user_id, timestamp DESC);
        CREATE INDEX IF NOT EXISTS idx_audit_action_time ON user_actions_log_global_global(action_type, timestamp DESC);
        CREATE INDEX IF NOT EXISTS idx_audit_competition ON user_actions_log_global_global(competition_id, timestamp DESC);
        
        -- Índices para detecção de fraude
        CREATE INDEX IF NOT EXISTS idx_fraud_user_time ON fraud_detection_log_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global(user_id, timestamp DESC);
        CREATE INDEX IF NOT EXISTS idx_fraud_type_confidence ON fraud_detection_log_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global(detection_type, confidence_score DESC);
        CREATE INDEX IF NOT EXISTS idx_fraud_timestamp ON fraud_detection_log_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global(timestamp DESC);
        
        -- Índices compostos para queries complexas
        CREATE INDEX IF NOT EXISTS idx_complex_ranking ON competition_participants_global_global_optimized(
            competition_id, valid_invites_count DESC, fraud_invites_count ASC, joined_at ASC
        );
        CREATE INDEX IF NOT EXISTS idx_complex_user_activity ON users_global_global_optimized(
            is_blacklisted, fraud_score, last_activity DESC
        ) WHERE is_blacklisted = FALSE;
        """
        
        async with self.async_engine.begin() as conn:
            await session.execute(text(text(text(indexes_sql))
    ...
    async def _setup_partitioning(self):
        ...
        for i in range(12):
            ...
            partition_sql = f"""
            CREATE TABLE IF NOT EXISTS user_actions_log_global_global_y{year_month} 
            PARTITION OF user_actions_log_global_global
            FOR VALUES FROM ('{start_date}') TO ('{end_date}');
            """
            
            try:
                async with self.async_engine.begin() as conn:
                    await session.execute(text(text(text(partition_sql))
            except Exception as e:
        ...
        try:
            ...
            async with self.async_session_factory() as session:
                ...
                existing_query = text("""
                    ...
                    FROM global_global_unique_invited_users_global_global
                    WHERE invited_user_id = :invited_user_id 
                    AND inviter_user_id = :inviter_user_id 
                    AND competition_id = :competition_id
                """)
                
                result = await session.execute(text(existing_query, {
                ...
                result = await session.execute(text(suspicious_pattern_query, {
                    'invited_user_id': invited_user_id
                })
                ...
                # 3. Verificar se usuário está na "blacklist_global"
                blacklist_query = text("""
                    SELECT is_blacklisted, blacklist_reason, fraud_score
                    FROM users_global_global_optimized
                    WHERE user_id = :user_id
                """)
                
                result = await session.execute(text(blacklist_query, {'user_id': invited_user_id})
    ...
    async def _log_fraud_attempt(self, user_id: int, detection_type: str, result: FraudDetectionResult):
        ...
        try:
            async with self.async_session_factory() as session:
                log_query = text("""
                    INSERT INTO fraud_detection_log_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global 
        ...
        try:
            async with self.async_session_factory() as session:
                ranking_query = text("""
                    ...
                    FROM competition_participants_global_global_optimized cp
                    JOIN users_global_global_optimized u ON cp.user_id = u.user_id
                ...
                result = await session.execute(text(ranking_query, {
                    'competition_id': competition_id,
                    'limit': limit
                })
        ...
        try:
            async with self.async_session_factory() as session:
                ...
                unique_insert_query = text("""
                    INSERT INTO global_global_unique_invited_users_global_global 
                    ...
                    DO UPDATE SET 
                        join_count = global_global_unique_invited_users_global_global.join_count + 1,
                        last_join_timestamp = NOW(),
                        is_valid_invite = FALSE  -- Marcar como inválido se já existe
                    RETURNING is_valid_invite
                """)
                
                result = await session.execute(text(unique_insert_query, {
                    'invited_user_id': invited_user_id,
                    'inviter_user_id': inviter_user_id,
                ...
                participant_update_query = text("""
                    INSERT INTO competition_participants_global_global_optimized 
                    (competition_id, user_id, valid_invites_count, last_invite_at)
                    VALUES (:competition_id, :user_id, 1, NOW())
                    ON CONFLICT (competition_id, user_id)
                    DO UPDATE SET 
                        valid_invites_count = competition_participants_global_global_optimized.valid_invites_count + 1,
                        last_invite_at = NOW()
                """)
                
                await session.execute(text(participant_update_query, {
                    'competition_id': competition_id,
                ...
                user_update_query = text("""
                    UPDATE users_global_global_optimized 
                ...
                await session.execute(text(user_update_query, {'user_id': inviter_user_id})
                
                # 4. Atualizar uso do link
                link_update_query = text("""
                    UPDATE invite_links_global_global_optimized 
                    SET uses = uses + 1,
                        updated_at = NOW()
                    WHERE id = :link_id
                """)
                
                await session.execute(text(link_update_query, {'link_id': invite_link_id})
    ...
    async def get_performance_metrics(self) -> PerformanceMetrics:
        ...
        try:
            async with self.async_session_factory() as session:
                metrics_query = text("""
                    SELECT 
                        (SELECT COUNT(*) FROM users_global_global_optimized) as total_users_global_global,
                        (SELECT SUM(valid_invites_count) FROM competition_participants_global_global_optimized) as total_invites,
                ...
                return PerformanceMetrics(
                    ...
                    total_users_global_global=data.total_users_global_global or 0,
    ...
    async def cleanup_old_data(self, days_to_keep: int = 90):
        ...
        try:
            async with self.async_session_factory() as session:
                ...
                result = await session.execute(text(cleanup_query, {'days': days_to_keep})
