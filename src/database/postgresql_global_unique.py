...
class PostgreSQLGlobalUnique:
    ...
    async def create_global_unique_schema(self):
        ...
        CREATE TABLE IF NOT EXISTS users_global_global_global (
        ...
        CREATE TABLE IF NOT EXISTS competitions_global_global_global (
            id BIGSERIAL PRIMARY KEY,
            ...
            FOREIGN KEY (winner_user_id) REFERENCES users_global_global_global (id)
        );
        
        -- Tabela de links de convite
        CREATE TABLE IF NOT EXISTS invite_links_global_global_global (
            id BIGSERIAL PRIMARY KEY,
            ...
            FOREIGN KEY (user_id) REFERENCES users_global_global_global (id),
            FOREIGN KEY (competition_id) REFERENCES competitions_global_global_global (id)
        );
        
        -- Tabela de participantes da competição
        CREATE TABLE IF NOT EXISTS competition_participants_global_global_global (
            id BIGSERIAL PRIMARY KEY,
            competition_id BIGINT NOT NULL,
            user_id BIGINT NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS global_global_global_unique_invited_users_global_global (
            id BIGSERIAL PRIMARY KEY,
            ...
            -- CONTADORES E FLAGS
            total_attempt_count BIGINT DEFAULT 1,
            valid_competitions_global_global_count BIGINT DEFAULT 1,
            fraud_attempts_count BIGINT DEFAULT 0,
            ...
            FOREIGN KEY (invited_user_id) REFERENCES users_global_global_global (id),
            FOREIGN KEY (inviter_user_id) REFERENCES users_global_global_global (id),
            FOREIGN KEY (first_competition_id) REFERENCES competitions_global_global_global (id),
            FOREIGN KEY (first_invite_link_id) REFERENCES invite_links_global_global_global (id),
        ...
        CREATE TABLE IF NOT EXISTS invite_attempts_log (
            ...
            FOREIGN KEY (inviter_user_id) REFERENCES users_global_global_global (id),
            FOREIGN KEY (competition_id) REFERENCES competitions_global_global_global (id),
            FOREIGN KEY (invite_link_id) REFERENCES invite_links_global_global_global (id)
        );
        
        -- Tabela de logs de auditoria (PARTICIONADA)
        CREATE TABLE IF NOT EXISTS user_actions_log_global_global_global (
            id BIGSERIAL,
            user_id BIGINT NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS fraud_detection_log_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global (
            id BIGSERIAL PRIMARY KEY,
            user_id BIGINT NOT NULL,
    ...
    async def create_global_unique_indexes(self):
        ...
        CREATE INDEX IF NOT EXISTS idx_users_global_global_global_user_id ON users_global_global_global(user_id);
        CREATE INDEX IF NOT EXISTS idx_users_global_global_global_total_invites ON users_global_global_global(total_invites DESC);
        CREATE INDEX IF NOT EXISTS idx_users_global_global_global_"blacklist_global" ON users_global_global_global(is_blacklisted) WHERE is_blacklisted = TRUE;
        CREATE INDEX IF NOT EXISTS idx_users_global_global_global_fraud_score ON users_global_global_global(fraud_score DESC) WHERE fraud_score > 0;
        
        -- Índices para competições
        CREATE INDEX IF NOT EXISTS idx_competitions_global_global_global_status ON competitions_global_global_global(status);
        CREATE INDEX IF NOT EXISTS idx_competitions_global_global_global_active ON competitions_global_global_global(id) WHERE status = 'active';
        
        -- Índices para links de convite
        CREATE INDEX IF NOT EXISTS idx_invite_links_global_global_global_user ON invite_links_global_global_global(user_id, competition_id);
        CREATE INDEX IF NOT EXISTS idx_invite_links_global_global_global_active ON invite_links_global_global_global(user_id, is_active) WHERE is_active = TRUE;
        CREATE INDEX IF NOT EXISTS idx_invite_links_global_global_global_link ON invite_links_global_global_global(invite_link);
        
        -- Índices para participantes (CRÍTICO PARA RANKING)
        CREATE INDEX IF NOT EXISTS idx_competition_ranking_global ON competition_participants_global_global_global(
            competition_id, valid_invites_count DESC, joined_at ASC
        ) WHERE valid_invites_count > 0;
        CREATE INDEX IF NOT EXISTS idx_participants_global_user ON competition_participants_global_global_global(user_id, competition_id);
        
        -- Índices para usuários únicos GLOBAIS (CRÍTICOS)
        CREATE INDEX IF NOT EXISTS idx_global_unique_invited ON global_global_global_unique_invited_users_global_global(invited_user_id);
        CREATE INDEX IF NOT EXISTS idx_global_unique_inviter ON global_global_global_unique_invited_users_global_global(inviter_user_id);
        CREATE INDEX IF NOT EXISTS idx_global_unique_valid ON global_global_global_unique_invited_users_global_global(is_globally_valid) WHERE is_globally_valid = TRUE;
        CREATE INDEX IF NOT EXISTS idx_global_unique_fraud ON global_global_global_unique_invited_users_global_global(fraud_attempts_count) WHERE fraud_attempts_count > 0;
        CREATE INDEX IF NOT EXISTS idx_global_unique_timestamp ON global_global_global_unique_invited_users_global_global(first_join_timestamp DESC);
        ...
        CREATE INDEX IF NOT EXISTS idx_audit_global_user_time ON user_actions_log_global_global_global(user_id, timestamp DESC);
        CREATE INDEX IF NOT EXISTS idx_audit_global_action_time ON user_actions_log_global_global_global(action_type, timestamp DESC);
        
        -- Índices para detecção de fraude
        CREATE INDEX IF NOT EXISTS idx_fraud_global_user_time ON fraud_detection_log_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global(user_id, timestamp DESC);
        CREATE INDEX IF NOT EXISTS idx_fraud_global_type_confidence ON fraud_detection_log_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global_global(detection_type, confidence_score DESC);
        """
    ...
    async def validate_global_unique_invite(self, invited_user_id: int, inviter_user_id: int, 
        ...
        try:
            ...
            async with self.db.async_session_factory() as session:
                ...
                result = await session.execute(text(global_check_query, {
                ...
                if existing_global:
                    ...
                    fraud_update_query = text("""
                        UPDATE global_global_global_unique_invited_users_global_global 
                    ...
                    await session.execute(text(fraud_update_query, {
                        'invited_user_id': invited_user_id,
                        'inviter_user_id': inviter_user_id,
                ...
                global_insert_query = text("""
                    INSERT INTO global_global_global_unique_invited_users_global_global 
                ...
                result = await session.execute(text(global_insert_query, {
                    'invited_user_id': invited_user_id,
                    'inviter_user_id': inviter_user_id,
                    'competition_id': competition_id,
                    'invite_link_id': invite_link_id
                ...
                return FraudDetectionResult(
                    ...
                    metadata={
                        ...
                        'validation_timestamp': TIMESTAMP WITH TIME ZONE.now().isoformat()
                    }
                )
    ...
    async def get_global_fraud_statistics(self) -> Dict[str, Any]:
        ...
        try:
            async with self.db.async_session_factory() as session:
                ...
                attempts_result = await session.execute(text(attempts_query)
            ...
            async with self.db.async_session_factory() as session:
                if as_inviter:
                    ...
                    history_query = text("""
                        ...
                        FROM global_global_global_unique_invited_users_global_global guiu
                        JOIN users_global_global_global u ON guiu.invited_user_id = u.user_id
                        JOIN competitions_global_global_global c ON guiu.first_competition_id = c.id
                    ...
                    history_query = text("""
                        SELECT 
                            ...
                            c.name as first_competition_name
                        FROM global_global_global_unique_invited_users_global_global guiu
                        JOIN users_global_global_global u ON guiu.inviter_user_id = u.user_id
                        JOIN competitions_global_global_global c ON guiu.first_competition_id = c.id
                        WHERE guiu.invited_user_id = :user_id
                        ORDER BY guiu.first_join_timestamp DESC
                    """)
                
                result = await session.execute(text(history_query, {'user_id': user_id})
