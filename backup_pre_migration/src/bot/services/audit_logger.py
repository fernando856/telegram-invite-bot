...
class AuditLogger:
    ...
    async def log_blacklist_action(self, user_id: int, action: str, reason: str,
        ...
        message = f"Usuário {user_id} {'adicionado à' if action == 'add' else 'removido da'} blacklist"
        if auto_generated:
            message += " (automático)"
    ...
    async def _flush_buffer(self):
        ...
        try:
            async with self.db.db.async_session_factory() as session:
                ...
                # Inserção em lote
                insert_query = text("""
                    INSERT INTO user_actions_log_global 
                ...
                await session.execute(insert_query, log_data)
    ...
    async def get_audit_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.db.db.async_session_factory() as session:
                ...
                result = await session.execute(stats_query, {'days': days})
                ...
                type_result = await session.execute(type_stats_query, {'days': days})
                ...
                level_result = await session.execute(level_stats_query, {'days': days})
    ...
    async def cleanup_old_logs(self) -> Dict[str, int]:
        ...
        try:
            ...
            async with self.db.db.async_session_factory() as session:
                ...
                await session.execute(text("VACUUM ANALYZE user_actions_log_global"))
