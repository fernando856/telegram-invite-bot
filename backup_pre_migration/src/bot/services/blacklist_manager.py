"""
Sistema de Blacklist Automática para Fraudadores
...
class BlacklistReason(Enum):
    """Razões para blacklist automático"""
...
@dataclass
class BlacklistRule:
    """Regra para blacklist automático"""
    name: str
    condition: str
    threshold: float
    action: str
...
class BlacklistManager:
    """
    Gerenciador de blacklist automática
    ...
    def __init__(self):
        ...
        # Regras de blacklist automático
        ...
        self.cache_last_update = datetime.min
    ...
    async def check_and_apply_auto_blacklist(self, user_id: int, 
        ...
        """
        try:
            logger.info(f"🔍 Verificando auto-blacklist para usuário {user_id} (trigger: {trigger_event})")
    ...
    async def _check_multiple_fraud_attempts(self, user_id: int) -> Optional[BlacklistEntry]:
        ...
        try:
            async with self.db.db.async_session_factory() as session:
                ...
                result = await session.execute(fraud_query, {'user_id': user_id})
            ...
            async with self.db.db.async_session_factory() as session:
                    ...
                    coordinated_indicators AS (
                        SELECT 
                            ...
                            (
                                ...
                                FROM global_unique_invited_users guiu2
                                WHERE DATE_TRUNC('minute', guiu2.first_join_timestamp) = utc.time_bucket
                            ) as total_users_in_minute
                        FROM user_time_clusters utc
                    )
                    SELECT 
                        MAX(total_users_in_minute) as max_coordinated_users,
                        COUNT(*) as suspicious_time_buckets
                    FROM coordinated_indicators
                    WHERE total_users_in_minute > 5
                """)
                
                result = await session.execute(coordinated_query, {'user_id': user_id})
                data = result.fetchone()
                
                max_coordinated = data.max_coordinated_users or 0
                ...
                if max_coordinated >= rule['threshold'] and suspicious_buckets >= 2:
                    return BlacklistEntry(
                        ...
                        confidence=rule['confidence'],
                        details={
                            'max_coordinated_users': max_coordinated,
    ...
    async def _check_bot_behavior(self, user_id: int) -> Optional[BlacklistEntry]:
        ...
        try:
            async with self.db.db.async_session_factory() as session:
                ...
                result = await session.execute(bot_query, {'user_id': user_id})
                data = result.fetchone()
                ...
                if bot_score >= rule['threshold']:
                    ...
                    if not rule['permanent']:
                        expires_at = datetime.now() + timedelta(days=rule['duration_days'])
    ...
    async def _check_rapid_pattern(self, user_id: int) -> Optional[BlacklistEntry]:
        ...
        try:
            async with self.db.db.async_session_factory() as session:
                ...
                result = await session.execute(rapid_query, {'user_id': user_id})
                data = result.fetchone()
    ...
    async def _apply_blacklist(self, user_id: int, blacklist_entry: BlacklistEntry) -> Optional[BlacklistEntry]:
        """Aplica blacklist no banco de dados"""
        try:
            async with self.db.db.async_session_factory() as session:
                from sqlalchemy import text
                
                # Atualizar usuário na blacklist
                ...
                # Log da ação de blacklist
                log_query = text("""
                    INSERT INTO fraud_detection_log_global 
                ...
                logger.warning(f"🚫 BLACKLIST APLICADO: Usuário {user_id} - {blacklist_entry.reason.value}")
                return blacklist_entry
    ...
    async def is_blacklisted(self, user_id: int) -> bool:
        ...
        try:
            ...
            async with self.db.db.async_session_factory() as session:
                ...
                result = await session.execute(blacklist_query, {'user_id': user_id})
                data = result.fetchone()
    ...
    async def _remove_from_blacklist(self, user_id: int):
        """Remove usuário da blacklist (quando expira)"""
        try:
            async with self.db.db.async_session_factory() as session:
                from sqlalchemy import text
                ...
                await session.execute(remove_query, {'user_id': user_id})
                await session.commit()
                ...
                logger.info(f"✅ Usuário {user_id} removido da blacklist (expiração)")
    ...
    async def manual_blacklist(self, user_id: int, reason: str, admin_id: int, 
        ...
        """Blacklist manual por administrador"""
        try:
            expires_at = None if permanent else datetime.now() + timedelta(days=duration_days)
            ...
            logger.error(f"Erro no blacklist manual: {e}")
            return False
